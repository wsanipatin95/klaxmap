...
export class BkpSchedulesComponent implements OnInit, PendingChangesAware{
  ...
  cargar(){this.loading.set(true);this.error.set(null);this.this.repo.listarSchedules(this.q(),0,100,null).pipe(finalize(()=>this.loading.set(false))).subscribe({next:(r:any)=>{this.items.set(r.items??[]); if(!this.selected()&&this.items().length)this.seleccionar(this.items()[0]);},error:e=>this.error.set(e?.message||'No se pudo cargar')});}
  ...
  guardar(){let payload=parseJsonObject(this.jsonText()); const id=Number(this.selected()?.[this.idField]??0); delete (payload as any)[this.idField]; this.saving.set(true); const req=this.mode()==='crear'?this.this.repo.crearSchedule(payload):this.this.repo.editarSchedule(id,payload); req.pipe(finalize(()=>this.saving.set(false))).subscribe({next:(res:any)=>{this.notify.success(this.title+' guardado',res.mensaje);this.success.set(res.mensaje);this.dirty.set(false);this.cargar();},error:e=>{this.error.set(e?.message||'No se pudo guardar');this.notify.error('No se pudo guardar',e?.message);}});}
  async eliminar(item=this.selected()){if(!item)return; const ok=await this.confirm.confirmDelete(item.nombre||`${this.title} #${item[this.idField]}`); if(!ok)return; this.saving.set(true); this.this.repo.eliminarSchedule(Number(item[this.idField])).pipe(finalize(()=>this.saving.set(false))).subscribe({next:(res:any)=>{this.notify.success(this.title+' eliminado',res.mensaje);this.selected.set(null);this.dirty.set(false);this.cargar();},error:e=>this.notify.error('No se pudo eliminar',e?.message)});}
