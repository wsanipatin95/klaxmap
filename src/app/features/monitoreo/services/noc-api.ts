...
export class NocApi {
  ...
  // ---- ACS (TR-069): router del cliente, por contrato ----
  acsStatus(contrato: string): Observable<any> { return this.http.get<ApiEnvelope<any>>(`${API}/acs/cpe/${encodeURIComponent(contrato)}`).pipe(map(unwrap)); }
  acsTareas(contrato: string): Observable<any[]> { return this.http.get<ApiEnvelope<any[]>>(`${API}/acs/cpe/${encodeURIComponent(contrato)}/tareas`).pipe(map(unwrap)); }
  ...
  ztePortHistory(id: number, port: string, hours = 6): Observable<any[]> { return this.http.get<ApiEnvelope<any[]>>(`${API}/zte/olts/${id}/ports/history?port=${encodeURIComponent(port)}&hours=${hours}`).pipe(map(unwrap)); }
  ...
  supInbox(estado?: number | null, area?: number | null, mio?: number | null, limit = 200): Observable<any[]> {
    let u = `${API}/support/inbox?limit=${limit}`;
    if (estado != null) u += `&estado=${estado}`;
    if (area != null) u += `&area=${area}`;
    if (mio != null) u += `&mio=${mio}`;
    return this.http.get<ApiEnvelope<any[]>>(u).pipe(map(unwrap));
  }
  ...
  diagSnmpGet(ip: string, community: string, oid: string, port = 161): Observable<string> {
    return this.http.get(`${API}/diag/snmpget?ip=${encodeURIComponent(ip)}&community=${encodeURIComponent(community)}&oid=${encodeURIComponent(oid)}&port=${port}`, { responseType: 'text' });
  }
  diagSnmpWalk(ip: string, community: string, oid: string, port = 161, limit = 40): Observable<string> {
    return this.http.get(`${API}/diag/snmpwalk?ip=${encodeURIComponent(ip)}&community=${encodeURIComponent(community)}&oid=${encodeURIComponent(oid)}&port=${port}&limit=${limit}`, { responseType: 'text' });
  }
  diagScanColumns(ip: string, community: string, base: string, from = 1, to = 30, samples = 3, port = 161, pauseMs = 250): Observable<string> {
    return this.http.get(`${API}/diag/scan-columns?ip=${encodeURIComponent(ip)}&community=${encodeURIComponent(community)}&base=${encodeURIComponent(base)}&from=${from}&to=${to}&samples=${samples}&port=${port}&pauseMs=${pauseMs}`, { responseType: 'text' });
  }
  supTomar(id: number): Observable<any> { return this.http.post<ApiEnvelope<any>>(`${API}/support/inbox/${id}/tomar`, {}).pipe(map(unwrap)); }
  supEstado(id: number, estado: number, nota?: string): Observable<any> { return this.http.post<ApiEnvelope<any>>(`${API}/support/inbox/${id}/estado`, { estado, nota }).pipe(map(unwrap)); }
  supNota(id: number, nota: string): Observable<any> { return this.http.post<ApiEnvelope<any>>(`${API}/support/inbox/${id}/nota`, { nota }).pipe(map(unwrap)); }
  supInboxResumen(area?: number | null): Observable<any[]> {
    const u = area != null ? `${API}/support/inbox/resumen?area=${area}` : `${API}/support/inbox/resumen`;
    return this.http.get<ApiEnvelope<any[]>>(u).pipe(map(unwrap));
  }
