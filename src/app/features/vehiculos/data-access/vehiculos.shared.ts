...
export type CrudPatchRequest<ID = number> = {
  id: ID;
...
export function buildListParams(query: ListQuery = {}): HttpParams {
  ...
  if (query.q != null && String(query.q).trim() !== '') params = params.set('q', String(query.q).trim());
...
export function toTimestamp(value?: string | null, defaultHour = '00:00:00'): string | null {
  if (!value) return null;
  const trimmed = value.trim();
  if (!trimmed) return null;
  if (trimmed.includes('T') || trimmed.includes(' ')) return trimmed;
  return `${trimmed} ${defaultHour}`;
}

export function normalizePatchId<ID = number>(id: ID, cambios: Record<string, unknown>): CrudPatchRequest<ID> {
  return { id, cambios };
}
