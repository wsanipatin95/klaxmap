...
STATE_BLOCK = """  readonly estadoFilters: EstadoFilterOption[] = [
  ...
  readonly servicioFilterOptions = [
    'REPARACION',
    'MANTENIMIENTO',
    'DIAGNOSTICO',
    'GARANTIA',
...
COMPUTED_BLOCK = """  readonly ordenesFiltradas = computed(() => {
    ...
    let items = this.ordenes();

    if (filter !== 'TODOS') {
      items = items.filter((item) => this.matchesEstadoFilter(item.estadoOrden, filter));
    }

    if (this.filterServicio) {
      items = items.filter((item) => String(item.tipoServicio || '').toUpperCase() === this.filterServicio);
    }

    if (this.filterFechaDesde) {
      items = items.filter((item) => this.ordenDateValue(item) >= this.filterFechaDesde);
    }

    if (this.filterFechaHasta) {
      items = items.filter((item) => this.ordenDateValue(item) <= this.filterFechaHasta);
    }

    if (this.filterSoloActivas) {
      items = items.filter((item) => !this.isOrdenBloqueada(item));
    }
...
METHODS_BLOCK = """  setEstadoFilter(filter: OrdenEstadoFilter) {
  ...
  activeFilterCount(): number {
    ...
    if (this.estadoFilter() !== 'TODOS') count += 1;
    if (this.q.trim()) count += 1;
    if (this.filterServicio) count += 1;
    if (this.filterFechaDesde) count += 1;
    if (this.filterFechaHasta) count += 1;
    if (this.filterSoloActivas) count += 1;
  ...
  private ordenDateValue(item: VehOrdenTrabajo): string {
    const raw = String(item.fechaIngreso || item.fecGen || '').trim();
    return raw ? raw.slice(0, 10) : '0000-00-00';
  }
...
TOP_HTML = """<div class=\"vehPageShell vehPageShell--ordenes\">
  ...
  <section class=\"compactSearchRail\">
    <div class=\"compactSearchRail__main\">
      ...
      <div class=\"filterPopover\" *ngIf=\"filtrosAvanzadosVisible()\">
        <div class=\"filterPopover__head\">
          <strong>Más filtros</strong>
          <span>{{ ordenesFiltradas().length }} OT visibles</span>
        </div>

        <div class=\"filterPopover__grid\">
          <div class=\"filterField\">
            <label>Servicio</label>
            <select [(ngModel)]=\"filterServicio\" [ngModelOptions]=\"{ standalone: true }\"
              (ngModelChange)=\"onAdvancedFilterChange()\">
              <option value=\"\">Todos</option>
              <option *ngFor=\"let option of servicioFilterOptions\" [value]=\"option\">{{ option }}</option>
            </select>
          </div>

          <div class=\"filterField\">
            <label>Solo activas</label>
            <select [(ngModel)]=\"filterSoloActivas\" [ngModelOptions]=\"{ standalone: true }\"
              (ngModelChange)=\"onAdvancedFilterChange()\">
          ...
          <div class=\"filterField\">
            ...
            <input type=\"date\" [(ngModel)]=\"filterFechaDesde\" [ngModelOptions]=\"{ standalone: true }\"
              (ngModelChange)=\"onAdvancedFilterChange()\">
          </div>

          <div class=\"filterField\">
            <label>Ingreso hasta</label>
            <input type=\"date\" [(ngModel)]=\"filterFechaHasta\" [ngModelOptions]=\"{ standalone: true }\"
              (ngModelChange)=\"onAdvancedFilterChange()\">
          </div>
        </div>

        <div class=\"filterPopover__summary\">
          <span class=\"filterChip\">Estado: {{ estadoFilterText() }}</span>
          <span class=\"filterChip\">Texto: {{ q.trim() || 'Sin texto' }}</span>
          <span class=\"filterChip\" *ngIf=\"filterServicio\">Servicio: {{ filterServicio }}</span>
          <span class=\"filterChip\" *ngIf=\"filterFechaDesde\">Desde: {{ filterFechaDesde }}</span>
          <span class=\"filterChip\" *ngIf=\"filterFechaHasta\">Hasta: {{ filterFechaHasta }}</span>
          <span class=\"filterChip\" *ngIf=\"filterSoloActivas\">Solo activas</span>
        </div>
      </div>
    </div>
  ...
  <div class=\"ordenesLayout ordenesLayout--dense\">
          ...
          <div class=\"orderRowTop\">
            ...
            <div class=\"orderRowActions\">
              <button
                ...
                icon=\"pi pi-pencil\"
                title=\"Editar\"
                [disabled]=\"isOrdenBloqueada(item)\"
                (click)=\"editarOrden(item); $event.stopPropagation()\">
              </button>
              ...
              <button
                ...
                title=\"Finalizar\"
                [disabled]=\"!canFinalizeOrden(item)\"
                (click)=\"finalizarOrden(item); $event.stopPropagation()\">
              </button>
              ...
              <button
                ...
                icon=\"pi pi-replay\"
                title=\"Devolver\"
                [disabled]=\"!canDevolverOrden(item)\"
                (click)=\"devolverOrden(item); $event.stopPropagation()\">
              </button>
              ...
              <button
                ...
                title=\"Anular\"
                [disabled]=\"isOrdenBloqueada(item)\"
                (click)=\"anularOrden(item); $event.stopPropagation()\">
              </button>
              ...
              <button
                ...
                title=\"Eliminar\"
                [disabled]=\"isOrdenBloqueada(item)\"
                (click)=\"eliminarOrden(item); $event.stopPropagation()\">
              </button>
            </div>
          </div>
    ...
    <section class=\"workspaceCard workspaceCard--unified\">
      <div class=\"selectedOrderBar\" *ngIf=\"selectedOrden() as ordenActual\">
        <div class=\"selectedOrderBar__copy\">
          <div class=\"selectedOrderBar__title\">
            ...
            <span
              class=\"stateInlinePill\"
              [class.is-danger]=\"isOrdenAnulada(ordenActual)\"
              [class.is-success]=\"isOrdenFinalizada(ordenActual)\"
              [class.is-process]=\"!isOrdenAnulada(ordenActual) && !isOrdenFinalizada(ordenActual)\">
              {{ ordenActual.estadoOrden || 'RECIBIDO' }}
            </span>
          </div>

          <span class=\"selectedOrderBar__meta\">
            Ingreso: {{ formatDateTimeFriendly(ordenActual.fechaIngreso || ordenActual.fecGen) }}
          </span>
        </div>
        ...
        <div class=\"selectedOrderBar__actions\">
          <button
            ...
            [disabled]=\"!canFinalizeOrden(ordenActual)\"
            (click)=\"finalizarOrden(ordenActual)\">
          </button>
          ...
          <button
            ...
            [disabled]=\"!canDevolverOrden(ordenActual)\"
            (click)=\"devolverOrden(ordenActual)\">
          </button>
      ...
      <app-orden-detail-panel
        ...
        [vehiculoNombre]=\"vehiculoNombreSeleccionado()\"
        [responsableRecepcionNombre]=\"responsableRecepcionNombre()\"
        [responsableTecnicoNombre]=\"responsableTecnicoNombre()\"
        [checklist]=\"checklist()\"
        ...
        [ordenFacturas]=\"ordenFacturas()\"
        [vistas]=\"vistas()\"
        [selectedVista]=\"selectedVista()\"
        [facturasOt]=\"facturasOt()\"
        [selectedFacturaOt]=\"selectedFacturaOt()\"
        [facturaDetalle]=\"facturaDetalle()\"
        [cobrosFactura]=\"cobrosFactura()\"
        [workflowResultado]=\"workflowResultado()\"
        [checklistLabelMap]=\"checklistLabelMap()\"
        [trabajoLabelMap]=\"trabajoLabelMap()\"
        [articuloLabelMap]=\"articuloLabelMap()\"
        [articulosCatalogo]=\"articulosCatalogo()\"
        [readonlyMode]=\"isOrdenBloqueada(selectedOrden())\"
        [garantias]=\"garantias()\"
        [selectedGarantia]=\"selectedGarantia()\"
        [garantiaDetalles]=\"garantiaDetalles()\"
        [garantiaMovimientos]=\"garantiaMovimientos()\"
        (saveChecklist)=\"guardarChecklistMasivo($event)\"
        (saveTrabajo)=\"guardarTrabajoWorkbench($event)\"
        (saveHallazgo)=\"guardarHallazgoWorkbench($event)\"
        (saveFoto)=\"guardarFotoWorkbench($event)\"
        (saveRepuesto)=\"guardarRepuestoWorkbench($event)\"
        (deleteRepuesto)=\"eliminarRepuestoWorkbench($event)\"
        (articuloQueryChange)=\"onRepuestoArticuloQueryChange($event)\"
        (articuloSelected)=\"seleccionarArticuloRepuesto($event)\"
        (selectHallazgo)=\"seleccionarHallazgo($event)\"
        (clearHallazgoSelection)=\"limpiarSeleccionHallazgoWorkbench()\"
        (selectVista)=\"seleccionarVista($event)\"
        (selectFactura)=\"seleccionarFacturaOt($event)\"
        (pointMarked)=\"createMarcaDesdeCanvas($event)\"
        (createFactura)=\"guardarFacturaDesdePanel($event)\"
        (openCobro)=\"abrirComercialDrawer('cobro')\"
        (openContabilizar)=\"abrirComercialDrawer('contabilizar')\"
        (selectGarantia)=\"seleccionarGarantia($event)\"
        (openGarantia)=\"abrirGarantiaDrawer()\"
        (openGarantiaDetalle)=\"abrirGarantiaDetalleDrawer()\"
        (openGarantiaMovimiento)=\"abrirGarantiaMovimientoDrawer()\">
      </app-orden-detail-panel>
    </section>
  </div>
  ...
  <ng-template #noOrdenes>
    <app-vehiculos-empty-state
      ...
      title=\"Sin órdenes\"
      subtitle=\"No hay órdenes para el estado o búsqueda aplicada.\">
    </app-vehiculos-empty-state>
  </ng-template>

  """

DETAIL_HEAD = """<ng-container *ngIf=\"orden; else noSelection\">
...
def backup(path: Path) -> None:
    bkp = path.with_suffix(path.suffix + '.bak-redisenio-v2')
    if not bkp.exists():
...
def replace_or_insert_types(ts: str) -> str:
    pattern = re.compile(r"type OrdenEstadoFilter = .*?type EstadoFilterOption = \{.*?\};\n\n", re.S)
    if pattern.search(ts):
        return pattern.sub(TYPES_BLOCK, ts, count=1)
    marker = "type AtributoRowForm = FormGroup<{"
    if marker not in ts:
        raise RuntimeError('No se encontró punto para insertar tipos de filtro.')
    return ts.replace(marker, TYPES_BLOCK + marker, 1)


def replace_or_insert_state(ts: str) -> str:
    pattern = re.compile(
        r"  readonly estadoFilters: EstadoFilterOption\[\] = \[.*?\n  filtrosAvanzadosVisible = signal\(false\);(?:\n  private filterVersion = signal\(0\);)?(?:\n\n  filterServicio = '';\n  filterFechaDesde = '';\n  filterFechaHasta = '';\n  filterSoloActivas = false;)?\n\n",
        re.S,
    )
    if pattern.search(ts):
        return pattern.sub(STATE_BLOCK, ts, count=1)
    marker = "  readonly TIPO_FACTURACION_OPTIONS = ['PARCIAL', 'TOTAL'];\n"
    if marker not in ts:
        raise RuntimeError('No se encontró TIPO_FACTURACION_OPTIONS.')
    return ts.replace(marker, marker + "\n" + STATE_BLOCK, 1)


def replace_or_insert_computed(ts: str) -> str:
    pattern = re.compile(r"  readonly ordenesFiltradas = computed\(\(\) => \{.*?\n  \}\);\n\n", re.S)
    if pattern.search(ts):
        return pattern.sub(COMPUTED_BLOCK, ts, count=1)
    marker = re.compile(
        r"  readonly articulosCatalogo = computed<VehArticuloCatalogo\[\]>\(\(\) =>\n    Object\.values\(this\.articuloCache\(\)\)\.sort\(\(a, b\) =>\n      String\(a\.articulo \|\| ''\)\.localeCompare\(String\(b\.articulo \|\| ''\)\),\n    \),\n  \);\n",
        re.S,
    )
    m = marker.search(ts)
    if not m:
        raise RuntimeError('No se encontró articulosCatalogo.')
    return ts[:m.end()] + "\n" + COMPUTED_BLOCK + ts[m.end():]


def replace_cargar(ts: str) -> str:
    pattern = re.compile(r"  cargar\(\) \{\n    this\.loading\.set\(true\);.*?\n  \}\n\n  seleccionarOrden", re.S)
    if not pattern.search(ts):
        raise RuntimeError('No se encontró método cargar().')
    return pattern.sub(CARGAR_BLOCK + "\n  seleccionarOrden", ts, count=1)


def replace_or_insert_methods(ts: str) -> str:
    pattern = re.compile(r"  setEstadoFilter\(filter: OrdenEstadoFilter\) \{.*?  private ordenDateValue\(item: VehOrdenTrabajo\): string \{.*?\n  \}\n\n", re.S)
    if pattern.search(ts):
        return pattern.sub(METHODS_BLOCK, ts, count=1)

    old_pattern = re.compile(r"  setEstadoFilter\(filter: OrdenEstadoFilter\) \{.*?  private normalizeEstadoFilter\(rawEstado: string \| null \| undefined\): string \{.*?\n  \}\n\n", re.S)
    if old_pattern.search(ts):
        return old_pattern.sub(METHODS_BLOCK, ts, count=1)

    marker = "  seleccionarOrden(item: VehOrdenTrabajo) {\n    this.selectedOrden.set(item);\n    this.cargarDetalle(item);\n  }\n\n"
    if marker not in ts:
        raise RuntimeError('No se encontró seleccionarOrden().')
    return ts.replace(marker, marker + METHODS_BLOCK, 1)
...
def patch_ts() -> None:
    ...
    backup(path)
    ts = read(path)
    ts = replace_or_insert_types(ts)
    ts = replace_or_insert_state(ts)
    ts = replace_or_insert_computed(ts)
    ts = replace_cargar(ts)
    ts = replace_or_insert_methods(ts)
    write(path, ts)
...
def patch_ordenes_html() -> None:
    ...
    if idx == -1:
        raise RuntimeError('No se encontró app-orden-main-form-drawer.')
    html = TOP_HTML + html[idx:]
    write(path, html)
...
def patch_detail_html() -> None:
    ...
    tab_idx = html.find('    <section class="tabRail">')
    if tab_idx == -1:
        tab_idx = html.find('    <section class="tabRail')
    if tab_idx == -1:
        raise RuntimeError('No se encontró tabRail en orden-detail-panel.component.html.')

    html = DETAIL_HEAD + html[tab_idx:]

    summary_pattern = re.compile(
        r"    <section class=\"panelCard(?: panelCard--summary)?\" \*ngIf=\"activeTab\(\)==='resumen'\">.*?\n    </section>\n\n    <section class=\"panelCard\" \*ngIf=\"activeTab\(\)==='checklist'\">",
        re.S,
    )
    if not summary_pattern.search(html):
        raise RuntimeError('No se encontró sección resumen en orden-detail-panel.component.html.')
    html = summary_pattern.sub('    ' + SUMMARY_BLOCK + '    <section class="panelCard" *ngIf="activeTab()===\'checklist\'">', html, count=1)
    write(path, html)
...
def patch_detail_scss() -> None:
    ...
    backup(path)
    scss = read(path)
    marker = '/* Ajustes compactos v2 ordenes */'
    if marker in scss:
        scss = scss[:scss.index(marker)].rstrip() + '\n'
    scss = scss.rstrip() + DETAIL_SCSS_APPEND + '\n'
    write(path, scss)
...
def main() -> int:
    ...
    print('[OK] Rediseño de órdenes v2 aplicado.')
    print('[OK] Corrige: barra compacta, filtros en popover, items uniformes y resumen sin espacios grandes.')
    return 0
