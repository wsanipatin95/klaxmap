...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, ButtonModule, VehiculosFormDrawerComponent],
...
export class OrdenReportesPanelComponent {
  ...
  get hasSelectedOrder(): boolean { return !!this.orden?.idVehOrdenTrabajo; }
  ...
  closeBlankDrawer() { this.drawerVisible.set(false); }
  ...
  descargarBlank() {
    ...
    this.descargarDocumento(formato, 'blank', { idVehTipoVehiculo: formato === 'trabajo' ? idVehTipoVehiculo : undefined }, true);
  }
  ...
  private syncTipoRequirement(formato: VehReporteFormato) {
    ...
    if (formato === 'trabajo') control.addValidators(Validators.required);
    else { control.clearValidators(); control.setValue(null); }
  ...
  private ensureTiposVehiculoLoaded() {
    ...
    this.repo.listarTipos('', 0, 200, true).subscribe({
      next: (res) => {
        this.tiposVehiculo.set((res.items ?? []).slice().sort((a, b) => String(a.tipoVehiculo || '').localeCompare(String(b.tipoVehiculo || ''))));
  ...
  private descargarDocumento(formato: VehReporteFormato, modo: VehReporteModo, extra: { idVehOrdenTrabajo?: number | null; idVehTipoVehiculo?: number | null | undefined }, closeAfter = false) {
  ...
  private handleDownload(response: HttpResponse<Blob>, fallbackName: string) {
    ...
    if (!blob) { this.notify.warn('Sin archivo', 'El backend no devolvió contenido para descargar.'); return; }
