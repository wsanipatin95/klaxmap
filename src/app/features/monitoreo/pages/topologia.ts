...
@Component({
  ...
  styles: [`
    ...
    .hud { position:absolute; top:12px; left:12px; z-index:6; display:flex; gap:8px; flex-wrap:wrap; align-items:center; pointer-events:none; }
    .hud .chip { box-shadow:0 1px 6px rgba(11,34,57,.12); }
    .hud-note { position:absolute; top:52px; left:12px; z-index:6; background:#fff; border:1px solid var(--border); border-radius:10px;
      padding:6px 12px; font-size:12.5px; color:#334; box-shadow:0 3px 12px rgba(11,34,57,.14); display:inline-flex; align-items:center; gap:10px; max-width:74%; }
    .hud-note.lnk { border-color:#93c5fd; background:#eff6ff; color:#1a56c4; }
    ...
    .toolbar .btn { height:28px; padding:0 12px; border-radius:16px; display:inline-flex; align-items:center; gap:5px; font-size:12.5px; }
    .ham { border:none; background:none; cursor:pointer; width:38px; height:38px; border-radius:9px; display:inline-flex; align-items:center; justify-content:center; color:#334; font-size:18px; }
    .ham:hover { background:#eef1f4; }
    .autochip { height:28px; display:inline-flex; align-items:center; gap:5px; padding:0 12px; border-radius:16px; background:#e6f7ec; color:#16794a; font-size:12px; font-weight:600; white-space:nowrap; }
  `],
  template: `
    ...
    <div class="top">
      ...
      <span class="toolbar" style="margin-left:auto">
        ...
        <span class="autochip" title="Monitoreo de caídas automático">🟢 Auto {{ autoSecs }}s</span>
      </span>
    </div>
    ...
    <div class="canvas-outer">
      ...
      @if (linkFrom(); as lf) {
        <div class="hud-note lnk">🔗 Conectando desde <b>{{ lf.deviceName }} · {{ lf.portLabel }}</b> — abrí otro equipo y tocá "Conectar aquí".
        ...
        <svg #svg [attr.viewBox]="'0 0 ' + W + ' ' + H" [attr.width]="W * zoom()" [attr.height]="H * zoom()"
          ...
          @for (n of nodes(); track n.id) {
            <g [attr.transform]="'translate(' + n.x + ',' + n.y + ')'" [style.cursor]="connectMode() ? 'pointer' : 'grab'" (mousedown)="onNodeDown($event, n)">
              ...
              <text x="12" y="21" font-size="12" font-weight="700" fill="#0b2239" [attr.textLength]="nameLen(n)" lengthAdjust="spacingAndGlyphs">{{ ic(n) }} {{ n.name }}</text>
              <text x="12" y="37" font-size="10.5" fill="#5b6b7b">{{ n.ip_address || '— sin IP —' }}</text>
              <rect x="12" y="45" [attr.width]="statusText(n) === 'Online' ? 58 : 54" height="14" rx="7" [attr.fill]="chipFill(n)"></rect>
              <circle cx="21" cy="52" r="3.2" [attr.fill]="stroke(n)"></circle>
              <text x="28" y="55" font-size="9.5" font-weight="700" [attr.fill]="stroke(n)">{{ statusText(n) }}</text>
              <text [attr.x]="NW - 12" y="55" font-size="9" fill="#9aa8b6" text-anchor="end">{{ n.device_type }}</text>
            </g>
          }
        </svg>
      </div>
...
export class Topologia implements OnInit, OnDestroy {
  ...
  onEsc() { if (this.fullscreen()) this.fullscreen.set(false); }

  ngOnInit() {
    // Solo lee y refresca el estado (ping/caídas) cada autoSecs. La sincronización con el ERP la hace
    // el scheduler de fondo (cada ~5 min) y el botón Importar — NO en cada entrada, para no arrastrar
  ...
  private layout(nodes: TopoNode[]) {
    ...
    const rowH = 64 + 46;                       // alto de tarjeta + separación
    ...
    for (const t of types) {
      ...
      y += Math.ceil(arr.length / perRow) * rowH + 26;   // separación entre grupos de tipo
    }
  ...
  portInfo(l: any): string {
    ...
    if (a || b) return (a || '?') + ' ↔ ' + (b || '?') + ' · click para borrar';
    return 'Enlace sin puertos definidos · reconéctalo por puerto · click para borrar';
  }
  nameLen(n: TopoNode): number | null {
    const chars = (n.name || '').length + 2;   // +icono
    const w = chars * 7;
    const max = this.NW - 18;
    return w > max ? max : null;                // solo limita si se pasaría del ancho
  }
  ...
  private connectNodeClick(n: TopoNode) {
    // Conexión SIEMPRE por puerto: abre el panel del equipo para elegir el puerto a enlazar.
    this.openDevice(n);
